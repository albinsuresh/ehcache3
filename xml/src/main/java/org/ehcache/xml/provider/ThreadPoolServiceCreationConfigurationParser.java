...
class ThreadPoolServiceCreationConfigurationParser<T extends ServiceCreationConfiguration<?>> extends SimpleCoreServiceCreationConfigurationParser<ThreadPoolReferenceType, T> {
  ...
  ThreadPoolServiceCreationConfigurationParser(Class<T> configType,
    ...
    super(configType, getter, setter, config -> parser.apply(config.getThreadPool()), config -> {
      ThreadPoolReferenceType threadPoolReferenceType = new ThreadPoolReferenceType();
      threadPoolReferenceType.setThreadPool(unparser.apply(config));
      return threadPoolReferenceType;
    });
  }
}
