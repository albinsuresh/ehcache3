...
public class CacheManagerPersistenceConfigurationParserTest {
  ...
  public void parseServiceCreationConfiguration() throws SAXException, JAXBException, ParserConfigurationException, IOException, ClassNotFoundException {
    final URL resource = XmlConfigurationTest.class.getResource("/configs/disk-persistent-cache.xml");
    ConfigurationParser rootParser = new ConfigurationParser(resource.toExternalForm());

    CacheManagerPersistenceConfigurationParser parser = new CacheManagerPersistenceConfigurationParser();
    ...
    assertThat(configuration, instanceOf(CacheManagerPersistenceConfiguration.class));

    CacheManagerPersistenceConfiguration providerConfiguration = (CacheManagerPersistenceConfiguration) configuration;
    assertThat(providerConfiguration.getRootDirectory(), is(new File("some/dir")));
